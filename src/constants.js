// export const BASE_URL=import.meta.env.VITE_NODE_ENV==='development'?'http://localhost:5000':'';
export const BASE_URL='https://store-crm-backend.vercel.app';
export const PRODUCTS_URL='/api/products'
export const USERS_URL='/api/users'
export const ORDERS_URL='/api/orders'
export const UPLOADS_URL='/api/upload'
export const UPLOADS_IMGTWOURL='/api/upload/imgtwo'
export const UPLOADS_IMGTHREEURL='/api/upload/imgthree'
export const UPLOADS_BANNERIMG='/api/upload/bannerimg'
export const UPLOADS_BANNERIMGTWO='/api/upload/bannerimgtwo'
export const UPLOADS_BANNERIMGTHREE='/api/upload/bannerimgthree'
